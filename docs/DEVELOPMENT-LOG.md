...
Chronological record of protocol reverse-engineering, dead ends, and fixes, moved out of
the README in 0.1.26 to keep that file about using the extension rather than building it.

Kept rather than deleted because most entries record *why* something is the way it is —
the frame length field, the MTU negotiation, the IBM Plex weight mapping — and that
reasoning is the expensive part to rediscover. Newest sections are at the bottom.
...
## README restructure (2026-09-07, 0.1.26)

The README had grown to 1409 lines, almost all of it this log, and still opened by
describing the project as a standalone test app that was "deliberately **not** a karoo-ext
extension yet" — untrue for a long time. Someone arriving at the repo had no way to learn
what the extension does or how to set it up without reading a protocol debugging diary
first.

The README is now about using the extension: what it does, how cameras and profiles
relate, a worked setup walkthrough, building, and known limitations. Everything historical
moved here rather than being deleted, because most of it records *why* something is the way
it is — the frame length field, the MTU negotiation, the IBM Plex weight mapping — and that
reasoning is far more expensive to rediscover than to keep.
